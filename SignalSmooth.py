...
def smooth(x,window_len=11,window='hanning'):
    ...
    sampleRate=numpy.r_[x[window_len-1:0:-1],x,x[-1:-window_len:-1]]
    #print(len(sampleRate))
    if window == 'flat': #moving average
        waveData=numpy.ones(window_len,'d')
    else:
        waveData=eval('numpy.'+window+'(window_len)')
    
    y=numpy.convolve(waveData/waveData.sum(),sampleRate,mode='valid')
    return y    
...
def smooth_demo():    
    ...
    hold(True)
    for waveData in windows[1:]:
        eval('plot('+waveData+'(ws) )')
    ...
    plot(x)
    plot(xn)
    for waveData in windows:
        plot(smooth(xn,10,waveData))
